...
var userPianoCounts = 0;
var userPianoNotes = [];
var userPianoTimes = [];
var instructions = 'Notes can be clicked using the mouse or by pressing the keys noted (for example, pressing "Q" is equivalent to hitting the lowest key). When you are done composing, click "Submit." Clicking "Start Over" will clear all previous notes from memory.';
var audioPath = "https://staff.washington.edu/sbng/metrical/audio/"
var allSounds = [
...
var noteNames = {
	81:'C3',50:'C#3',87:'D3',51:'D#3',
	69:'E3',82:'F3',53:'F#3',84:'G3',
	54:'G#3',89:'A3',55:'A#3',85:'B3',
	73:'C4',57:'C#4',79:'D4',48:'D#4',
	80:'E4',90:'F4',83:'F#4',88:'G4',
	68:'G#4',67:'A4',70:'A#4',86:'B4'};
...
function realPiano(){
	var one = '<p id="prompt">Compose a melody for: <div id="highlight">\
		Mary had a little lamb</div></p>';
	var text = "<div id='buttons'><div id='buttons'><button onclick='instruct()' id='ib'>\
		Instructions</button><button onclick=clearPress()>Start Over</button>\
		<button onclick='submit()' id='submit'>Submit</button></div><div id='response'></div>";
	document.addEventListener('keyup',notePress);
	document.addEventListener('click',clickPress);
	document.body.innerHTML = one+piano+text;
}
...
function submit(){
	var x = 0;
	var len = userPianoTimes.length
	while(x < len){ 
    	userPianoTimes[x] = Math.round(userPianoTimes[x]-userPianoTimes[0]);
    	if (typeof userPianoNotes[x]== "number"){
    		userPianoNotes[x] = noteNames[userPianoNotes[x]];
    	}
    	x++;
	}

	if (userPianoCounts >0){
		document.getElementById('response').innerHTML = '<p>Notes: ['+
			userPianoNotes.join(', ')+']</p><p>Count: '+userPianoCounts+
			'</p><p>Timings (ms): ['+userPianoTimes.join(', ')+']</p>';		
	}
	else {
...
function notePress(e){
		try {
			...
			userPianoCounts++;
			userPianoNotes.push(e.keyCode);
			userPianoTimes.push(e.timeStamp);
			} 
		catch (error) {} 
}
